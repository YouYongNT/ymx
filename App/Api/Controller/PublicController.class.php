...
class PublicController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        // 所有图片路径
        define(__DATAURL__, $http_type . $_SERVER['SERVER_NAME'] . ':8080' . __DATA__ . '/');
    ...
    public function ispc($val)
    {
        ...
        $url = $app->where('id=' . $val)
            ->field('ispcshop,end_time,name,pcnav_color,ahover_color')
            ->find();
        // print_r($url);exit;
        // return $url;
    ...
    public function getuserdiscount($uid)
    {
        $discount = M('user')->alias("i")
            ->join('left join lr_user_group as u on i.group_id=u.id')
            ->where([
            'i.id' => intval($uid)
        ])
            ->field('u.discount as dis')
            ->find();
        if ($discount['dis']) {
            if ($discount['dis'] > 0)
                return $discount['dis'] / 100;
            else
                return 1;
        } else
            return 1;
