...
class ProductController extends PublicController
{
    ...
    public function index()
    {
        ...
        if ($pro['pro_buff']) { // 如果产品属性有值才进行数据组装
            ...
            foreach ($pro_buff as $key => $val) {
                ...
                $guigelist = M('guige')->where("attr_id=" . intval($val) . ' AND pid=' . intval($pro['id']))
                    ->field("id,name")
                    ->select();
                $ggss = array();
                $gg = array();
    ...
    public function make_order()
    {
        ...
        $uid = 1;
        // 获得产品信息
        $pro = M('product')->field('id,shop_id,photo_x,name,price,price_yh')
            ->where('id=' . intval($pro_id) . ' AND del=0 AND is_down=0')
            ->find();
        $pro['photo_x'] = __DATAURL__ . $pro['photo_x'];
        if (! $pro) {
            echo json_encode(array(
