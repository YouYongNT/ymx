...
class WxpayController extends Controller{
	...
	public function wxpay(){
		...
		$input->SetBody("亚马逊商学院购买_".trim($order_info['order_sn']));
		$input->SetAttach("亚马逊商学院购买_".trim($order_info['order_sn']));
		$input->SetOut_trade_no($pay_sn);
		$input->SetTotal_fee(floatval($order_info['amount'])*100);
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 3600));
		$input->SetGoods_tag("亚马逊商学院购买_".trim($order_info['order_sn']));
		$input->SetNotify_url('https://ymx.91youyong.cn/index.php/Api/Wxpay/notify');
