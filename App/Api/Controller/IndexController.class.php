...
class IndexController extends PublicController {
    ...
    public function index(){
        ...
        //首页培训课程
        //======================
        $course = M('course')->where('del=0')->order('id desc')->field('id')->select();
    ...
    public function getlist(){
        ...
        $cou_list = M('product')->where("del=0 AND pro_type=1 AND is_down=0 AND cid = $COU")->order('sort desc,id desc')->limit($limit.',8')->select();
        foreach ($cou_list as $k => $v) {
        	$cou_list[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
        	$cou_list[$k]['price'] = floor($v['price']);
        	$cou_list[$k]['price_yh'] = floor($v['price_yh']);
        	$cou_list[$k]['price_dl'] = floor($v['price_dl']);
        }

        echo json_encode(array('prolist'=>$pro_list,'courlist'=>$cou_list),JSON_UNESCAPED_UNICODE);
        exit();
    }
