...
<body>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 管理员管理 <span class="c-gray en">&gt;</span> 管理员列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);"
        ...
        <table class="table table-border table-bordered table-bg">
            <thead>
                ...
                <tr class="text-c">
                    ...
                    <th width="100">状态</th>
                    <th width="130">会员开始时间</th>
                    <th width="130">会员结束时间</th>
            ...
            <tbody id="news_option">
                ...
                <?php if(is_array($userlist)): $i = 0; $__LIST__ = $userlist;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($v["id"]); ?>" data-name="<?php echo ($v["name"]); ?>" class="text-c">
                        <td><?php echo ($v["id"]); ?></td>
                        <td><img src="<?php echo ($v["photo"]); ?>" width="65px" height="65px" /></td>
                        <td><?php echo ($v["name"]); ?></td>
                        <td><?php echo ($v["tel"]); ?></td>
                        <td>
                            <?php if($v["del"] != 0): ?><label style="color:red;">已禁用</label>
                            <?php else: ?>
                            	<label style="color:green;">正常</label><?php endif; ?>
                        </td>
                        <td><?php echo ($v["vip_starttime"]); ?></td>
                        <td><?php echo ($v["vip_endtime"]); ?></td>
                        <td><?php echo ($v["addtime"]); ?></td>
                        <td class="obj_1">
                          	<a href="<?php echo U('User/add');?>?id=<?php echo ($v["id"]); ?>">修改</a> | 
                            <a onclick='del_id_urls(<?php echo ($v["id"]); ?>,<?php echo ($page); ?>)'>
                                <?php if($v["del"] != 0): ?><label style="color:green;">恢复</label>
                            	<?php else: ?>
                            		禁用<?php endif; ?>
                            </a> | 
                            <a onclick='dodel_id_urls(<?php echo ($v["id"]); ?>,<?php echo ($page); ?>)'>删除</a>
                        </td>
    ...
    <script>
        ...
        function dodel_id_urls(id, page) {
            if (confirm('你确定要删除该用户所有信息吗？')) {
                location.href = '<?php echo U("dodel");?>?did=' + id + '&page=' + page;
            }
        }
