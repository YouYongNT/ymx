...
<head>
    ...
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/js/jquery.XYTipsWindow.min.2.8.js"></script>
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/js/mydate.js"></script>
    <link href="/xcx_yamaxun/Public/admin/css/order.css" rel="stylesheet" type="text/css" />

    <title>全部订单</title>
</head>

<body>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span class="c-gray en">&gt;</span> 全部订单 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);"
    ...
    <div class="page-container">
        ...
        <form name='form' action="<?php echo U('index');?>" method='get'>
            <div class="pro_4 bord_1">
                <div class="pro_5">
                    支付类型：
                    <select class="inp_1 inp_6 input-text" name="pay_type" id="type" style="width:100px;">
                ...
                <div class="pro_5">
                    订单状态：
                    <select class="inp_1 inp_6 input-text" name="pay_status" id="status" style="width:100px;">
                ...
                <div class="pro_5">
                    购买时间：
                    <input class="inp_1 inp_6 input-text" id="start_time" name="start_time" value="<?php echo $start_time ?>" onfocus="MyCalendar.SetDate(this)" style="width:160px;">
                    <input class="inp_1 inp_6 input-text" id="end_time" name="end_time" value="<?php echo $end_time ?>" onfocus="MyCalendar.SetDate(this)" style="width:160px;">
                    <input class="btn btn-success" type="button" value="搜 索" onclick="product_option();">
                </div>
                <div class="pro_6">
        ...
        <table class="table table-border table-bordered table-bg">
            ...
            <?php if(is_array($order_list)): $i = 0; $__LIST__ = $order_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($order["id"]); ?>" data-name="<?php echo ($order["name"]); ?>" class="text-c">
                    ...
                    <td>
                        <?php if($order["type"] == 'alipay'): ?>支付宝
                            <?php elseif($order["type"] == 'weixin'): ?>微信支付
                            <?php else: ?>现金支付<?php endif; ?>
                    </td>
                    <td class="status">
                        <?php if($order["back"] == 1): ?><font style="color:red">申请退款</font>
                            <?php elseif($order["back"] == 2): ?>
