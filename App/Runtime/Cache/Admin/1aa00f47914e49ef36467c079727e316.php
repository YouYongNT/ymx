...
<body>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品管理 <span class="c-gray en">&gt;</span> 全部产品 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);"
            title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">
        <div class="text-c">
            <input type="text" class="input-text" style="width:250px" placeholder="产品名称" id="name" value="<?php echo ($name); ?>">
            <select class="inp_1 inp_6 input-text" id="tuijian" style="width:100px;vertical-align:middle;">
        ...
        <table class="table table-border table-bordered table-bg">
            ...
            <tbody id="news_option">
                ...
                <?php if(is_array($productlist)): $i = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
                        <td><?php echo ($v["id"]); ?></td>
                        <td style="padding:3px 0;"><img src="/xcx_yamaxun/Data/<?php echo ($v["photo_x"]); ?>" width="65px" height="65px" /></td>
                        <td><?php echo ($v["name"]); ?></td>
                        <td><?php echo ($v["price_yh"]); ?></td>
                        <!-- <td>
                            <p id="new_<?php echo ($v["id"]); ?>">
                                <?php if($v["is_show"] == 1): ?><a class="label blue" onclick="pro_new(<?php echo ($v["id"]); ?>,1);">新品上市<?php else: ?><a class="label err" onclick="pro_new(<?php echo ($v["id"]); ?>,0);">非新品<?php endif; ?></a></p>
                            <p id="hot_<?php echo ($v["id"]); ?>" style="margin-top:5px;">
                                <?php if($v["is_hot"] == 1): ?><a class="label succ" onclick="pro_hot(<?php echo ($v["id"]); ?>,1);">热卖商品<?php else: ?><a class="label err" onclick="pro_hot(<?php echo ($v["id"]); ?>,0);">非热卖<?php endif; ?></a></p>
                        ...
                        <!-- <td>
                            <?php if($v["type"] == 1): ?><label style="color:green;">推荐</label><?php endif; ?>
                        </td> -->
                        <td>
    ...
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/static/h-ui.admin/js/H-ui.admin.js"></script>
    <!--/_footer 作为公共模版分离出去-->

    <!--请在下方写此页面业务相关的脚本-->
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/lib/My97DatePicker/4.8/WdatePicker.js"></script>
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/lib/datatables/1.10.0/jquery.dataTables.min.js"></script>
    <script type="text/javascript" src="/xcx_yamaxun/Public/admin/lib/laypage/1.2/laypage.js"></script>
    <script>
        ...
        function pro_new(pro_id, type) {
            ...
            $.post("<?php echo U('Product/set_new');?>", {
                pro_id: pro_id
            }, function(data) {
                if (data.status == 1) {
                    if (type == 1) {
        ...
        function pro_hot(pro_id, type) {
            ...
            $.post("<?php echo U('Product/set_hot');?>", {
                pro_id: pro_id
            }, function(data) {
                if (data.status == 1) {
                    if (type == 1) {
        ...
        function pro_zk(pro_id, type) {
            ...
            $.post("<?php echo U('Product/set_zk');?>", {
                pro_id: pro_id
            }, function(data) {
                if (data.status == 1) {
                    if (type == 1) {
