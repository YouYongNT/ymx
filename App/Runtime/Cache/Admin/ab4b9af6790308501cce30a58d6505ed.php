...
<body>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 品牌管理 <span class="c-gray en">&gt;</span> 全部品牌 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);"
        ...
        <table class="table table-border table-bordered table-bg">
            ...
            <tbody id="news_option">
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$b): $mod = ($i % 2 );++$i;?><tr class="text-c">
                        <td><?php echo ($b["id"]); ?></td>
                        <td><img src="/xcx_yamaxun/Data/<?php echo ($b["photo"]); ?>" width="65px" height="65px"></td>
                        <td><?php echo ($b["name"]); ?></td>
                        <td>
                            <?php if($b["type"] == 1): ?><label style="color:green;">是</label>
                                <?php else: ?>否<?php endif; ?>
                        </td>
                        <td class="obj_1">
                            <a href="<?php echo U('set_tj');?>?id=<?php echo ($b["id"]); ?>">推荐</a> |
                            <a href="<?php echo U('add');?>?id=<?php echo ($b["id"]); ?>">修改</a> | <a onclick="del_id_url(<?php echo ($b["id"]); ?>)">删除</a>
                        </td>
