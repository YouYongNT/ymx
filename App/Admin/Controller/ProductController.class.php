	...
	public function add(){
		...
		if($_POST['submit']==true){
			...
			if (!empty($_POST['relate_id'])){
				$relate_array = array();
				$relate_id = $_POST['relate_id'];
				foreach ($relate_id as $key=>$rid){
					$relate_array[$rid] = $_POST['count'][$key]; //array[$key] = $count;$key 为被关联产品ID，$count为数量
				}
				$array['relate'] = serialize($relate_array);
			}
		...
		//查询关联产品
		if ($pro_allinfo['relate'] != ''){
			$relate_list = array();
			$relate = unserialize($pro_allinfo['relate']);
			foreach ($relate as $proid=>$count){
				$pinfo = M('product as p')->join('lr_category as c on p.cid=c.id')->where('p.id='.$proid)->field('p.id,p.name,c.name as catname')->find();
				$relate_list[$proid] = array('id'=>$proid,'name'=>$pinfo['name'],'catname'=>$pinfo['catname'],'count'=>$count);
			}
